...
// @namespace      http://userscripts.org/users/oneduality
...
0.43 (2013.11.21)
        - Bug fix ( thanks mwaldstein)

0.42 (2013.11.13)
	- Fixed bugs introduced in 0.41 ( I hope )
...
0.38
     (2013.01.11)
        - Lowered how many times the script attempts to expand more stories to test performance.
0.37 (2013.01.11)
	- delete counter is re-implemented
        - Fixed bug so that clear time line options only appear on the activity log screen
0.36 (2013.01.11)
        - HTTPS support!
0.35 (2013.01.10)
...
// Convert a hash to a query string for posting
	...
	function remove(myministory, mydata)
	{
		...
		if($.browser.chrome){
			$.ajax({
				type    : "POST",
				url     : protocol+"www.facebook.com/ajax/timeline/take_action_on_story.php?",
				data    : mydata,
				...
				if(selected!="")
				{
					...
					switch (selected) 
					{
						...
						case "DeleteActivityOlder":
							...
							{
								youngerThan = prompt("Delete all activities younger than ? days (0 = No limit)",0);
							}
